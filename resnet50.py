...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False, norm_layer=None):
        ...
        self.classifier1 = nn.Linear(2048, 2)
...
class ResNetHH(nn.Module):

    def __init__(self, block, layers, zero_init_residual=False, norm_layer=None):
        super(ResNetHH, self).__init__()
        self.feature_maps = OrderedDict()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self.inplanes = 64
        ...
        self.bn1 = norm_layer(64)
        self.relu = nn.ReLU(inplace=True)
        self.add_conv = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.add_bn = norm_layer(64)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        out = self.classifier1(x)
        #self.feature_maps['block2'] = self.layer3[0].conv1(x).cpu().detach().numpy()
        return out
...
def resnet50H(pretrained=False, **kwargs):
    model = ResNetH(Bottleneck, [3, 4, 6, 3], **kwargs)
    return model

def resnet50HH(pretrained=False, **kwargs):
    model = ResNetHH(Bottleneck, [3, 4, 6, 3], **kwargs)
    return model
